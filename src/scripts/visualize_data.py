...
def get_html_for_transcript(i, chat, schema):
    ...
    for event in events:
        ...
        if event.action == 'message':
            ...
            if isinstance(event.data, dict):
                ordered_item = [(attr.name, item[attr.name]) for attr in schema.attributes]
            else:
                ordered_item = item
            chat_html.append("Selected " + ", ".join([x[1] for x in ordered_item]))
...
def aggregate_chats(transcripts, scenario_db, schema):
    ...
    for (idx, chat) in enumerate(transcripts):
        completed, chat_html = get_html_for_transcript(idx, chat, schema)
