...
class CraigslistSpider(Spider):
    ...
    def __init__(self, num_result_pages=1, num_item_per_page=120, cache_dir=None, from_cache=False, *args, **kwargs):
    ...
    def parse_item_page(self, response):
        if not self.from_cache and self.cache_dir:
            path = os.path.join(self.cache_dir, response.url.split('/')[-1])
