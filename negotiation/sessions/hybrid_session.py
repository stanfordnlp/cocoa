...
class HybridSession(object):
    @classmethod
    def get_session(cls, agent, kb, lexicon, config, generator):
        if kb.role == 'buyer':
            return BuyerHybridSession(agent, kb, lexicon, config, generator)
        elif kb.role == 'seller':
            return SellerHybridSession(agent, kb, lexicon, config, generator)
        else:
...
class BaseHybridSession(BaseRulebasedSession):
    def __init__(self, agent, kb, lexicon, config, generator):
        parser = Parser(agent, kb, lexicon)
        state = DialogueState(agent, kb)
        manager = None
        super(BaseHybridSession, self).__init__(agent, kb, parser, generator, manager, state, sample_temperature=10.)
        # even though self.state holds Utterance objects with LF objects

        self.lf_tokens = []    # stores logical form tokens
        self.kb = kb
        self.title = self.shorten_title(self.kb.facts['item']['Title'])
        self.config = default_config if config is None else config
        self.agent_to_role = Dialogue.get_role_mapping(agent, kb)
    ...
    def choose_action(self):
        # convert lf tokens into int
        lfs = [map(self.mappings['lf_vocab'].to_ind, lf) for lf in self.lf_tokens]
        batch = self.create_batch(lfs)
        encoder_init_state = None
        output_data = self.generator.generate_batch(batch)
        entity_tokens = self._output_to_tokens(output_data)
        action = entity_tokens[1] # first element is just start symbol
    ...
    def get_fraction(cls, zero, one, fraction):
        # see rulebased session for details
    ...
    def receive(self, event):
        # returns an Utterance object with LF attribute
        utterance = self.parser.parse(event, self.state)
        # returns a list of tokens which masquerades as an utterance
        ...
        intent = self.process_lf(self.agent)
        self.lf_tokens.append(intent)

        if self.bottomline is None:
            self.bottomline = self.estimate_bottomline()
        # self.dialogue.add_utterance(event.agent, utterance)

    def process_lf(self, agent):
        # extract the LF from the utterance attribute in the state to allow
        # long term access to context logical forms and direct access
        intent = state.utterance.lf['intent']  # -or- state.my_act
        intent.append(markers.EOS)
        # if it's a new turn
        if len(self.lf_tokens) == 0:
            # cat_symbol = category_to_marker[self.category_str]
            # intent.insert(0, cat_symbol)
            role = self.agent_to_role[agent]
            start_symbol = markers.GO_S if role == 'seller' else markers.GO_B
            intent.insert(0, start_symbol)
