...
bot-chat:
	PYTHONPATH=. python src/scripts/generate_dataset.py --schema-path data/friends-schema.json --scenarios-path output/friends-scenarios.json --test-examples-paths output/friends-bot-chat-$(model).json --train-max-examples 0 --test-max-examples 1 --agents neural neural --model-path $(checkpoint) --scenario-offset 1000
